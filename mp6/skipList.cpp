...
std::cout<<head_->nodePointers[0].next<<std::endl;
std::cout<<tail_<<std::endl;
std::cout<<tail_->nodePointers[0].next<<std::endl;

    head_->key = INT_MIN;
    head_->value = HSLAPixel(0, 0, 0, 0.5);

    tail_->key = INT_MAX;
    tail_->value = HSLAPixel(0, 0, 0, 0.5);
...
void SkipList::insert(int key, HSLAPixel value) {
	...
	if (key == nextKey){
          traverse->nodePointers[level].next->value = value;
          return;
        }
        if (key < nextKey) {
            level--;
...
//std::cout<<listHeight_<<std::endl;
//listHeight_++;
    // expand head_ and tail_ to encompass the new height of the list, if needed
if (listHeight_< newNodeLevel){
    for(int i = listHeight_; i < max(listHeight_, newNodeLevel); i++) {
//        std::cout<<listHeight_<<std::endl;
//        std::cout<<this->listHeight_<<std::endl;

//        std::cout<<"BEGIN!!!!"<<std::endl;
//        std::cout<<head_->nodePointers.size()<<std::endl;
//        gdbPrintNode(head_);
//        gdbPrintNode(tail_);
