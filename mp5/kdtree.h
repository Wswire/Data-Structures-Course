...
class KDTree
{
  ...
  public:
    ...
    int partition(vector<Point<Dim>>& copy, int start, int end, int dim);

    Point<Dim> quickSelect(vector<Point<Dim>>& copy, int start, int end, int median_idx, int dim);

    KDTreeNode* helper(vector<Point<Dim>>& copy, int start, int end, int dimCount);
    ...
    Point<Dim> findNearestHelper(KDTreeNode* subroot, const Point<Dim>& query, Point<Dim> currBest, int dimCount) const;
